---
title: AsposeArgumentException
second_title: Aspose.Email för .NET API-referens
description: Initierar en ny instans avAsposeArgumentExceptionaspose.email/asposeargumentexception class.
type: docs
weight: 10
url: /sv/net/aspose.email/asposeargumentexception/asposeargumentexception/
---
## AsposeArgumentException() {#constructor}

Initierar en ny instans av[`AsposeArgumentException`](../../asposeargumentexception) class.

```csharp
public AsposeArgumentException()
```

### Se även

* class [AsposeArgumentException](../../asposeargumentexception)
* namnutrymme [Aspose.Email](../../asposeargumentexception)
* hopsättning [Aspose.Email](../../../)

---

## AsposeArgumentException(string) {#constructor_2}

Initierar en ny instans av[`AsposeArgumentException`](../../asposeargumentexception) klass med ett specificerat felmeddelande.

```csharp
public AsposeArgumentException(string message)
```

| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| message | String | Meddelandet som beskriver felet. |

### Se även

* class [AsposeArgumentException](../../asposeargumentexception)
* namnutrymme [Aspose.Email](../../asposeargumentexception)
* hopsättning [Aspose.Email](../../../)

---

## AsposeArgumentException(string, string) {#constructor_5}

Initierar en ny instans av[`AsposeArgumentException`](../../asposeargumentexception) klass med ett specificerat felmeddelande.

```csharp
public AsposeArgumentException(string format, string parameter)
```

| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| format | String | Meddelandeformat |
| parameter | String |  |

### Se även

* class [AsposeArgumentException](../../asposeargumentexception)
* namnutrymme [Aspose.Email](../../asposeargumentexception)
* hopsättning [Aspose.Email](../../../)

---

## AsposeArgumentException(string, params object[]) {#constructor_4}

Initierar en ny instans av[`AsposeArgumentException`](../../asposeargumentexception) klass med ett specificerat felmeddelande.

```csharp
public AsposeArgumentException(string format, params object[] parameters)
```

| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| format | String | Meddelandeformat |
| parameters | Object[] | Formatparametrar |

### Se även

* class [AsposeArgumentException](../../asposeargumentexception)
* namnutrymme [Aspose.Email](../../asposeargumentexception)
* hopsättning [Aspose.Email](../../../)

---

## AsposeArgumentException(string, Exception) {#constructor_3}

Initierar en ny instans av[`AsposeArgumentException`](../../asposeargumentexception) klass med ett specificerat felmeddelande och en referens till det inre undantaget som är orsaken till detta undantag.

```csharp
public AsposeArgumentException(string message, Exception innerException)
```

| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| message | String | Felmeddelandet som förklarar orsaken till undantaget. |
| innerException | Exception | Undantaget som är orsaken till det aktuella undantaget, eller en nollreferens (Ingenting i Visual Basic) om inget inre undantag anges. |

### Se även

* class [AsposeArgumentException](../../asposeargumentexception)
* namnutrymme [Aspose.Email](../../asposeargumentexception)
* hopsättning [Aspose.Email](../../../)

---

## AsposeArgumentException(Exception) {#constructor_1}

Initierar en ny instans av[`AsposeArgumentException`](../../asposeargumentexception) klass med en referens till det inre undantaget som är orsaken till detta undantag.

```csharp
public AsposeArgumentException(Exception innerException)
```

| Parameter | Typ | Beskrivning |
| --- | --- | --- |
| innerException | Exception | Undantaget som är orsaken till det aktuella undantaget, eller en nollreferens (Ingenting i Visual Basic) om inget inre undantag anges. |

### Se även

* class [AsposeArgumentException](../../asposeargumentexception)
* namnutrymme [Aspose.Email](../../asposeargumentexception)
* hopsättning [Aspose.Email](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Email.dll -->
