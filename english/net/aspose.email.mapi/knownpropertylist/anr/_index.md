---
title: Anr
second_title: Aspose.Email for .NET API Reference
description: Contains a filter value used in ambiguous name resolution. Area Address Book Canonical name PidTagAnr Alternate names PR_ANR PR_ANR_A PR_ANR_W
type: docs
weight: 1090
url: /net/aspose.email.mapi/knownpropertylist/anr/
---
## KnownPropertyList.Anr field

Contains a filter value used in ambiguous name resolution. Area: Address Book Canonical name: PidTagAnr Alternate names: PR_ANR, PR_ANR_A, PR_ANR_W

```csharp
public static readonly PidTagPropertyDescriptor Anr;
```

### See Also

* class [PidTagPropertyDescriptor](../../pidtagpropertydescriptor)
* class [KnownPropertyList](../../knownpropertylist)
* namespace [Aspose.Email.Mapi](../../knownpropertylist)
* assembly [Aspose.Email](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Email.dll -->
