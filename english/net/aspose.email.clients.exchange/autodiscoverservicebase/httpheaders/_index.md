---
title: AutodiscoverServiceBase.HttpHeaders
second_title: Aspose.Email for .NET API Reference
description: AutodiscoverServiceBase property. Gets a collection of HTTP headers that will be sent with each request to EWS
type: docs
weight: 60
url: /net/aspose.email.clients.exchange/autodiscoverservicebase/httpheaders/
---
## AutodiscoverServiceBase.HttpHeaders property

Gets a collection of HTTP headers that will be sent with each request to EWS.

```csharp
public IDictionary<string, string> HttpHeaders { get; }
```

### See Also

* class [AutodiscoverServiceBase](../)
* namespace [Aspose.Email.Clients.Exchange](../../autodiscoverservicebase/)
* assembly [Aspose.Email](../../../)


