---
title: SmtpFailedRecipientException
second_title: Aspose.Email für .NET-API-Referenz
description: Initialisiert eine neue Instanz vonSmtpFailedRecipientExceptionaspose.email.clients.smtp/smtpfailedrecipientexception Klasse
type: docs
weight: 10
url: /de/net/aspose.email.clients.smtp/smtpfailedrecipientexception/smtpfailedrecipientexception/
---
## SmtpFailedRecipientException() {#constructor}

Initialisiert eine neue Instanz von[`SmtpFailedRecipientException`](../../smtpfailedrecipientexception) Klasse

```csharp
public SmtpFailedRecipientException()
```

### Siehe auch

* class [SmtpFailedRecipientException](../../smtpfailedrecipientexception)
* namensraum [Aspose.Email.Clients.Smtp](../../smtpfailedrecipientexception)
* Montage [Aspose.Email](../../../)

---

## SmtpFailedRecipientException(string) {#constructor_3}

Initialisiert eine neue Instanz von[`SmtpFailedRecipientException`](../../smtpfailedrecipientexception) Klasse

```csharp
public SmtpFailedRecipientException(string message)
```

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| message | String | Fehlermeldung |

### Siehe auch

* class [SmtpFailedRecipientException](../../smtpfailedrecipientexception)
* namensraum [Aspose.Email.Clients.Smtp](../../smtpfailedrecipientexception)
* Montage [Aspose.Email](../../../)

---

## SmtpFailedRecipientException(string, Exception) {#constructor_4}

Initialisiert eine neue Instanz von[`SmtpFailedRecipientException`](../../smtpfailedrecipientexception) Klasse

```csharp
public SmtpFailedRecipientException(string message, Exception innerException)
```

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| message | String | Fehlermeldung |
| innerException | Exception | Die Ausnahme, die die Ursache der aktuellen Ausnahme ist, oder ein Nullverweis (Nothing in Visual Basic), wenn keine innere Ausnahme angegeben ist. |

### Siehe auch

* class [SmtpFailedRecipientException](../../smtpfailedrecipientexception)
* namensraum [Aspose.Email.Clients.Smtp](../../smtpfailedrecipientexception)
* Montage [Aspose.Email](../../../)

---

## SmtpFailedRecipientException(SmtpStatusCode, string) {#constructor_1}

Initialisiert eine neue Instanz von[`SmtpFailedRecipientException`](../../smtpfailedrecipientexception) Klasse

```csharp
public SmtpFailedRecipientException(SmtpStatusCode statusCode, string failedRecipient)
```

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| statusCode | SmtpStatusCode | Statuscode |
| failedRecipient | String | Fehlerhafter Empfänger |

### Siehe auch

* enum [SmtpStatusCode](../../smtpstatuscode)
* class [SmtpFailedRecipientException](../../smtpfailedrecipientexception)
* namensraum [Aspose.Email.Clients.Smtp](../../smtpfailedrecipientexception)
* Montage [Aspose.Email](../../../)

---

## SmtpFailedRecipientException(SmtpStatusCode, string, string) {#constructor_2}

Initialisiert eine neue Instanz von[`SmtpFailedRecipientException`](../../smtpfailedrecipientexception) Klasse

```csharp
public SmtpFailedRecipientException(SmtpStatusCode statusCode, string failedRecipient, 
    string serverResponse)
```

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| statusCode | SmtpStatusCode | Statuscode |
| failedRecipient | String | Fehlerhafter Empfänger |
| serverResponse | String | Antwort des Servers |

### Siehe auch

* enum [SmtpStatusCode](../../smtpstatuscode)
* class [SmtpFailedRecipientException](../../smtpfailedrecipientexception)
* namensraum [Aspose.Email.Clients.Smtp](../../smtpfailedrecipientexception)
* Montage [Aspose.Email](../../../)

---

## SmtpFailedRecipientException(string, string, Exception) {#constructor_5}

Initialisiert eine neue Instanz von[`SmtpFailedRecipientException`](../../smtpfailedrecipientexception) Klasse

```csharp
public SmtpFailedRecipientException(string message, string failedRecipient, 
    Exception innerException)
```

| Parameter | Typ | Beschreibung |
| --- | --- | --- |
| message | String | Fehlermeldung |
| failedRecipient | String | Fehlerhafter Empfänger |
| innerException | Exception | Die Ausnahme, die die Ursache der aktuellen Ausnahme ist, oder ein Nullverweis (Nothing in Visual Basic), wenn keine innere Ausnahme angegeben ist. |

### Siehe auch

* class [SmtpFailedRecipientException](../../smtpfailedrecipientexception)
* namensraum [Aspose.Email.Clients.Smtp](../../smtpfailedrecipientexception)
* Montage [Aspose.Email](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Email.dll -->
