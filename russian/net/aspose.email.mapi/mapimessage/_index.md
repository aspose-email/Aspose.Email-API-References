...
| Имя | Описание |
| --- | --- |
| [MapiMessage](mapimessage#constructor)() | Инициализирует новый экземпляр класса[`MapiMessage`](../mapimessage). |
| [MapiMessage](mapimessage#constructor_1)(OutlookMessageFormat) | Инициализирует новый экземпляр класса[`MapiMessage`](../mapimessage). |
| [MapiMessage](mapimessage#constructor_2)(string, string, string, string) | Инициализирует новый экземпляр класса[`MapiMessage`](../mapimessage). |
| [MapiMessage](mapimessage#constructor_3)(string, string, string, string, OutlookMessageFormat) | Инициализирует новый экземпляр класса[`MapiMessage`](../mapimessage). |
...
| [ClientSubmitTime](../../aspose.email.mapi/mapimessage/clientsubmittime) { get; set; } | Получает или устанавливает дату и время отправитель сообщения отправил сообщение. |
...
| [DeliveryTime](../../aspose.email.mapi/mapimessage/deliverytime) { get; set; } | Получает или устанавливает дату и время сообщение было доставлено. |
| [DisplayBcc](../../aspose.email.mapi/mapimessage/displaybcc) { get; } | Получает список отображаемых имен всех получателей сообщений скрытой копии (BCC), разделенных точкой с запятой (;). |
...
| [Headers](../../aspose.email.mapi/mapimessage/headers) { get; set; } | Получает заголовки транспортных сообщений |
...
| [SentRepresentingEmailAddress](../../aspose.email.mapi/mapimessage/sentrepresentingemailaddress) { get; set; } | Получает или задает адрес электронной почты пользователя обмена сообщениями, представленного отправителем. |
...
| [SentRepresentingSmtpAddress](../../aspose.email.mapi/mapimessage/sentrepresentingsmtpaddress) { get; } | Получает или задает адрес электронной почты пользователя обмена сообщениями, представленного отправителем. |
...
| static [Load](../../aspose.email.mapi/mapimessage/load#load_1)(Stream, LoadOptions) | Загружает сообщение из потока с дополнительными параметрами. |
| static [Load](../../aspose.email.mapi/mapimessage/load#load_3)(string, LoadOptions) | Загружает сообщение из файла с дополнительными параметрами. |
...
| [GetCustomProperties](../../aspose.email.mapi/mapimessage/getcustomproperties)() | Получает набор пользовательских MapiProperties. |
...
| [SetStringPropertyValue](../../aspose.email.mapi/mapimessage/setstringpropertyvalue)(long, string) | Устанавливает значение строкового свойства. |
...
| [ToMapiMessageItem](../../aspose.email.mapi/mapimessage/tomapimessageitem)() | Преобразование MapiMessage в объект IMapiMessageItem в зависимости от MessageClass. |
...
| static [DestroyAttachments](../../aspose.email.mapi/mapimessage/destroyattachments)(string) | Уничтожает вложения в указанных файлах сообщений Outlook. DestroyAttachments игнорирует разбор вложений. |
...
Экземпляры класса MapiMessage используются для представления файлов документов Microsoft Outlook Message, которые анализируется классом MapiMessageReader. Чтобы получить доступ к отправителю, получателю и содержимому сообщения электронной почты, используйте связанные свойства класса MapiMessage.
...
apiMessage msg = MapiMessage.FromFile(@"c:\outlookmessage.msg");

/прочитать тему
nsole.WriteLine("Subject:" + msg.Subject);

/имя отправителя
nsole.WriteLine("From:" + msg.SenderName);

/сообщение body
nsole.WriteLine("Body:" + msg.Body);

/Вложения
reach(MapiAttachment att in msg.Attachments)
...
'Вложения
Для каждого att As MapiAttachment In msg.Attachments
Console.WriteLine("Имя вложения:" + att.FileName )
att.Save(att.FileName)
Next
```
