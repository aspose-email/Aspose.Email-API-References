---
title: SearchCondition
second_title: Справочник по Aspose.Email для .NET API
description: Задает условие для поисковых запросов
type: docs
weight: 1940
url: /ru/net/aspose.email.clients.activesync.transportlayer/searchcondition/
---
## SearchCondition class

Задает условие для поисковых запросов

```csharp
public class SearchCondition
```

## Конструкторы

| Имя | Описание |
| --- | --- |
| [SearchCondition](searchcondition)() | Конструктор по умолчанию. |

## Характеристики

| Имя | Описание |
| --- | --- |
| [Value](../../aspose.email.clients.activesync.transportlayer/searchcondition/value) { get; set; } | Строковое значение условия |

### Смотрите также

* пространство имен [Aspose.Email.Clients.ActiveSync.TransportLayer](../../aspose.email.clients.activesync.transportlayer)
* сборка [Aspose.Email](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Email.dll -->
