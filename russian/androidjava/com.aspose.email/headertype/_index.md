...
description: Представляет стандарты Интернета и RFC, определяющие поля заголовков, которые могут встречаться в интернет‑почтовых сообщениях.
type: docs
weight: 157
url: /ru/androidjava/com.aspose.email/headertype/
---
...
Представляет стандарты Интернета и RFC, определяющие поля заголовков, которые могут встречаться в интернет‑почтовых сообщениях.
## Methods

| Method | Описание |
| --- | --- |
| [equals(Object obj)](#equals-java.lang.Object-) | Возвращает логическое значение, указывающее, равен ли переданный объект obj этому. |
| [getApparentlyTo()](#getApparentlyTo--) | Вставляется при отправке электронного письма, когда в оригинальном сообщении отсутствует получатель 'To:'. |
| [getApprovedBy()](#getApprovedBy--) | Имя модератора почтовой рассылки, в которую отправлено это сообщение; необходимо при публикации, отправляемой в модерируемую рассылку, чтобы разрешить её распространение среди участников списка. |
| [getBcc()](#getBcc--) | Копия электронного письма, отправляемая одному или нескольким получателям без ведома основных получателей. |
| [getCC()](#getCC--) | Этот заголовок можно рассматривать как расширение поля 'To:', поскольку он используется для указания дополнительных получателей. |
| [getClass()](#getClass--) |  |
| [getComments()](#getComments--) | Это свободно формируемое поле заголовка, определённое в RFC2822. |
| [getContentTransferEncoding()](#getContentTransferEncoding--) | Третий из заголовков, связанных с MIME. |
| [getContentType()](#getContentType--) | 'Content-Type' определяет формат содержимого (кодировка символов и т.д.). Обратите внимание, что значения этого заголовка определяются по‑разному в RFC1049 и в MIME (RFC2045). |
| [getDate()](#getDate--) | Этот заголовок указывает дату (и время), обычно дату создания и отправки сообщения. |
| [getDispositionNotificationTo()](#getDispositionNotificationTo--) | Когда поле DispositionNotificationTo установлено, делается запрос на MDN (уведомление о доставке сообщения). |
| [getFollowupTo()](#getFollowupTo--) | Используется в Usenet News для указания, что дальнейшие обсуждения (=follow-up) статьи должны направляться в другой набор групп новостей, отличающийся от группы, к которой была направлена статья-ответ. |
| [getFrom()](#getFrom--) | Это поле содержит идентификацию лица(лиц), желавших отправить это сообщение. |
| [getImportance()](#getImportance--) | Получает важность. |
| [getInReplyTo()](#getInReplyTo--) | Ссылка на сообщение, к которому данное сообщение является ответом. |
| [getMIMEVersion()](#getMIMEVersion--) | Индикатор того, что это сообщение отформатировано согласно стандарту MIME, и указание, какая версия MIME используется. |
| [getMessageID()](#getMessageID--) | Уникальный идентификатор этого сообщения. |
| [getNewsgroups()](#getNewsgroups--) | В Usenet News: группа(ы), в которую был размещён этот материал. |
| [getReceived()](#getReceived--) | След MTAs, через которые прошло сообщение. |
| [getReferences()](#getReferences--) | Ссылка на другие связанные сообщения. |
| [getReplyTo()](#getReplyTo--) | Это поле заголовка предназначено для указания, куда отправитель хочет получать ответы. |
| [getReturnPath()](#getReturnPath--) | Используется для передачи информации из атрибута конверта MAIL FROM при окончательной доставке, когда сообщение покидает SMTP‑окружение, в котором используется 'MAIL FROM'. |
| [getReturnReceiptTo()](#getReturnReceiptTo--) | Отправитель может запросить подтверждение получения, включив это поле заголовка. |
| [getSender()](#getSender--) | Лицо или агент, отправляющий сообщение в сеть, если оно отличается от указанного в поле заголовка From:. |
| [getSensitivity()](#getSensitivity--) | Получает чувствительность. |
...
Ссылка на сообщение, к которому данное сообщение является ответом.
...
Лицо или агент, отправляющий сообщение в сеть, если оно отличается от указанного в поле заголовка From:. Должно быть аутентифицировано согласно RFC 822, но какой тип аутентификации не ясен.
...
Заголовок, тема, предмет. Часто используется как индикатор ветки для сообщений, отвечающих или комментирующих другие сообщения.
...
Основные получатели. Пример: To: someone@somedomain.com
...
Этот заголовок запрашивает автоматическое уведомление о подтверждении при получении или чтении сообщения. Обычно он игнорируется; предположительно какое‑то программное обеспечение обрабатывает его.
...
Информация о клиентском программном обеспечении отправителя. Пример: X-Mailer: Aspose.Email
...
**Returns:**
int - Хеш-код для текущего Object.
### notify() {#notify--}
```
...
| Parameter | Type | Описание |
| --- | --- | --- |
| type | [HeaderType](../../com.aspose.email/headertype) | Объект 'HeaderType' для преобразования. |

**Returns:**
java.lang.String — результат преобразования.
### wait() {#wait--}
```
