...
type: docs
weight: 362
url: /ru/androidjava/com.aspose.email/recurrencerule/
---
...
Frequency
```.
2. Укажите, как заканчивается шаблон повторения, используя ```
EndType
```,
 ```
Count
...
Обратите внимание, что если значения части правила ByXXX находятся за пределами доступных
область (например, BYMONTHDAY=30 в феврале), они просто игнорируются.

Информация, не содержащаяся в правиле, необходимая для определения различных экземпляров повторения
время начала и даты выводятся из ```
