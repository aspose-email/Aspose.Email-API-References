---
title: DebugAppender
second_title: Aspose.Email per riferimento all'API .NET
description: Rappresenta un appender abilitato solo durante il debug.
type: docs
weight: 20460
url: /it/net/aspose.email.tools.logging/debugappender/
---
## DebugAppender class

Rappresenta un appender abilitato solo durante il debug.

```csharp
public class DebugAppender : Appender
```

## Costruttori

| Nome | Descrizione |
| --- | --- |
| [DebugAppender](debugappender)() | Inizializza una nuova istanza di[`DebugAppender`](../debugappender) classe. |

## Proprietà

| Nome | Descrizione |
| --- | --- |
| [Formatter](../../aspose.email.tools.logging/appender/formatter) { get; set; } | Ottiene o imposta il formattatore. |

## Metodi

| Nome | Descrizione |
| --- | --- |
| override [Append](../../aspose.email.tools.logging/debugappender/append)(LogEntry) | Aggiunge la voce di registro specificata all'apper. |
| virtual [AppendHeader](../../aspose.email.tools.logging/appender/appendheader)() | Avvia il file di registro con un'intestazione specifica. |
| virtual [Close](../../aspose.email.tools.logging/appender/close)() | Chiude l'appendice. |
| [Dispose](../../aspose.email.tools.logging/appender/dispose)() | Rilascia le risorse non gestite utilizzate dall'appender. |
| virtual [Initialize](../../aspose.email.tools.logging/appender/initialize)() | Inizializza l'istanza appender. |

### Guarda anche

* class [Appender](../appender)
* spazio dei nomi [Aspose.Email.Tools.Logging](../../aspose.email.tools.logging)
* assemblea [Aspose.Email](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Email.dll -->
