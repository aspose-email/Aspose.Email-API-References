---
title: Save
second_title: Aspose.Email for .NET API 参考
description: 使用默认选项将此MapiContactaspose.email.mapi/mapicontact保存到 vCard 文件 支持的 vCard 版本为 2.1
type: docs
weight: 140
url: /zh/net/aspose.email.mapi/mapicontact/save/
---
## Save(string) {#save_3}

使用默认选项将此[`MapiContact`](../../mapicontact)保存到 vCard 文件。 支持的 vCard 版本为 2.1

```csharp
public void Save(string filePath)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| filePath | String | vCard 文件名 |

### 例外

| 例外 | （健康）状况 |
| --- | --- |
| ArgumentException | *filePath*is` null` 或` 空` |

### 也可以看看

* class [MapiContact](../../mapicontact)
* 命名空间 [Aspose.Email.Mapi](../../mapicontact)
* 部件 [Aspose.Email](../../../)

---

## Save(string, ContactSaveFormat) {#save_4}

将此[`MapiContact`](../../mapicontact)保存到使用默认选项的格式的指定文件中。 支持的保存格式是 vCard。

```csharp
public void Save(string filePath, ContactSaveFormat saveFormat)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| filePath | String | vCard 文件名 |
| saveFormat | ContactSaveFormat | 保存格式 |

### 例外

| 例外 | （健康）状况 |
| --- | --- |
| ArgumentException | *filePath*is` null` 或` 空` |
| NotSupportedException | 不支持指定格式 |

### 也可以看看

* enum [ContactSaveFormat](../../contactsaveformat)
* class [MapiContact](../../mapicontact)
* 命名空间 [Aspose.Email.Mapi](../../mapicontact)
* 部件 [Aspose.Email](../../../)

---

## Save(string, ContactSaveOptions) {#save_5}

使用指定的保存选项将此[`MapiContact`](../../mapicontact)保存到文件中。 支持的保存选项是[`VCardSaveOptions`](../../../aspose.email.personalinfo.vcard/vcardsaveoptions)

```csharp
public void Save(string filePath, ContactSaveOptions saveOptions)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| filePath | String | A vCard 文件名 |
| saveOptions | ContactSaveOptions | 保存选项 |

### 例外

| 例外 | （健康）状况 |
| --- | --- |
| ArgumentException | *filePath*是` null` 或` 空` |
| ArgumentNullException | *saveOptions*is` null` |
| NotSupportedException | 不支持某些保存选项 |

### 也可以看看

* class [ContactSaveOptions](../../contactsaveoptions)
* class [MapiContact](../../mapicontact)
* 命名空间 [Aspose.Email.Mapi](../../mapicontact)
* 部件 [Aspose.Email](../../../)

---

## Save(Stream) {#save}

将此[`MapiContact`](../../mapicontact)保存到具有 vCard 格式的给定流中。 支持的 vCard 版本为 2.1

```csharp
public void Save(Stream stream)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| stream | Stream | 要保存到 |

### 例外

| 例外 | （健康）状况 |
| --- | --- |
| ArgumentNullException | *stream*is` null` |
| NotSupportedException | *stream*不支持写入 |

### 也可以看看

* class [MapiContact](../../mapicontact)
* 命名空间 [Aspose.Email.Mapi](../../mapicontact)
* 部件 [Aspose.Email](../../../)

---

## Save(Stream, ContactSaveFormat) {#save_1}

将此[`MapiContact`](../../mapicontact)以使用默认选项的格式保存到给定的流。 支持的保存格式是 vCard

```csharp
public void Save(Stream stream, ContactSaveFormat saveFormat)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| stream | Stream | 要保存到 |
| saveFormat | ContactSaveFormat | 保存格式 |

### 例外

| 例外 | （健康）状况 |
| --- | --- |
| ArgumentNullException | *stream*is` null` |
| NotSupportedException | *stream*不支持写入 |
| NotSupportedException | 不支持指定格式 |

### 也可以看看

* enum [ContactSaveFormat](../../contactsaveformat)
* class [MapiContact](../../mapicontact)
* 命名空间 [Aspose.Email.Mapi](../../mapicontact)
* 部件 [Aspose.Email](../../../)

---

## Save(Stream, ContactSaveOptions) {#save_2}

使用指定的保存选项将此[`MapiContact`](../../mapicontact)保存到给定的流。 支持的保存选项是[`VCardSaveOptions`](../../../aspose.email.personalinfo.vcard/vcardsaveoptions)

```csharp
public void Save(Stream stream, ContactSaveOptions saveOptions)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| stream | Stream | A流保存到 |
| saveOptions | ContactSaveOptions | 保存选项 |

### 例外

| 例外 | （健康）状况 |
| --- | --- |
| ArgumentNullException | *stream*或*saveOptions*是` null` |
| NotSupportedException | *stream*不支持写入 |
| ArgumentException | 不正确*saveOptions* |
| NotSupportedException | 不支持某些保存选项 |

### 也可以看看

* class [ContactSaveOptions](../../contactsaveoptions)
* class [MapiContact](../../mapicontact)
* 命名空间 [Aspose.Email.Mapi](../../mapicontact)
* 部件 [Aspose.Email](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Email.dll -->
