...
description: 初始化一个新的实例OlmStorageaspose.email.storage.olm/olmstorageclass. 允许设置回调方法来处理 OLM 存储遍历期间发生的异常
type: docs
weight: 10
...
初始化一个新的实例[`OlmStorage`](../../olmstorage)class. 允许设置回调方法来处理 OLM 存储遍历期间发生的异常。
...
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| stream | Stream | 源流Stream与 OLM 存储数据。 |
