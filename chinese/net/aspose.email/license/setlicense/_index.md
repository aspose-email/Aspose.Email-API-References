---
title: SetLicense
second_title: Aspose.Email for .NET API 参考
description: 许可组件
type: docs
weight: 20
url: /zh/net/aspose.email/license/setlicense/
---
## SetLicense(string) {#setlicense_2}

许可组件。

```csharp
public void SetLicense(string licenseName)
```

### 评论

尝试在以下位置查找许可证:

1. 显式路径。

2. 组件的文件夹。

3. 客户端调用程序集的文件夹。

4. 入口程序集所在文件夹。

5. 客户端调用程序集中的嵌入资源。

**注意:** 在 .NET Compact Framework 上，尝试仅在这些位置查找许可证

1. 显式路径。

2. 客户端调用程序集中的嵌入资源。

2. 组件jar文件所在文件夹。

### 例子

在本例中，将尝试查找一个名为 MyLicense.lic 的许可证文件，位于包含  组件的文件夹中，在包含调用程序集的文件夹中， 在条目程序集的文件夹中，然后在调用程序集的嵌入资源中。

```csharp
[C#]

License license = new License();
license.SetLicense("MyLicense.lic");


[Visual Basic]

Dim license As License = New License
license.SetLicense("MyLicense.lic")
```

组件jar文件:

```csharp
License license = new License();
license.setLicense("MyLicense.lic");
```

可以是完整或短文件名 或嵌入的名称资源 。 使用空字符串切换到评估模式。

### 也可以看看

* class [License](../../license)
* 命名空间 [Aspose.Email](../../license)
* 部件 [Aspose.Email](../../../)

---

## SetLicense(Stream) {#setlicense_1}

许可组件。

```csharp
public void SetLicense(Stream stream)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| stream | Stream | 包含许可证的流。 |

### 评论

使用此方法从流中加载许可证。

### 例子

```csharp
[C#]

License license = new License();
license.SetLicense(myStream);


[Visual Basic]

Dim license as License = new License
license.SetLicense(myStream)

License license = new License();
license.setLicense(myStream);
```

### 也可以看看

* class [License](../../license)
* 命名空间 [Aspose.Email](../../license)
* 部件 [Aspose.Email](../../../)

---

## SetLicense(FileInfo) {#setlicense}

许可组件。

```csharp
public void SetLicense(FileInfo licenseFile)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| licenseFile | FileInfo | 许可证文件FileInfo。 |

### 例外

| 例外 | （健康）状况 |
| --- | --- |
| ArgumentNullException | *licenseFile*为空。 |
| FileNotFoundException | 找不到文件。 |
| UnauthorizedAccessException | 文件是只读的或者是目录。 |
| DirectoryNotFoundException | 指定的路径无效，例如位于未映射的驱动器上。 |
| IOException | 文件已经打开。 |

### 也可以看看

* class [License](../../license)
* 命名空间 [Aspose.Email](../../license)
* 部件 [Aspose.Email](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Email.dll -->
