...
description: 初始化LogEntryaspose.email.tools.logging/logentry类的新实例
type: docs
weight: 10
...
| --- | --- | --- |
| message | String | 要记录的消息正文。来自消息对象的 ToString() 方法的值。 |
| severity | LogLevel | 将条目严重性记录为[`Severity`](../severity)枚举。 （未指定、信息、警告或错误）。 |
...
创建[`LogEntry`](../../logentry)的新实例，带有完整的构造函数参数集
...
| --- | --- | --- |
| binaryDataMessage | Byte[] | 要记录的二进制消息体。 |
| messageEncoding | Encoding | 二进制消息编码 |
