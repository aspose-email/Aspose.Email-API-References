---
title: ObjectDisposedException
second_title: Aspose.Email for .NET API 参考
description: 初始化ObjectDisposedException类的新实例
type: docs
weight: 10
url: /zh/net/aspose.email.exceptions/objectdisposedexception/objectdisposedexception/
---
## ObjectDisposedException() {#constructor}

初始化ObjectDisposedException类的新实例。

```csharp
public ObjectDisposedException()
```

### 也可以看看

* class [ObjectDisposedException](../../objectdisposedexception)
* 命名空间 [Aspose.Email.Exceptions](../../objectdisposedexception)
* 部件 [Aspose.Email](../../../)

---

## ObjectDisposedException(string) {#constructor_1}

使用指定的错误消息初始化ObjectDisposedException类的新实例。

```csharp
public ObjectDisposedException(string message)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| message | String | 描述错误的消息。 |

### 也可以看看

* class [ObjectDisposedException](../../objectdisposedexception)
* 命名空间 [Aspose.Email.Exceptions](../../objectdisposedexception)
* 部件 [Aspose.Email](../../../)

---

## ObjectDisposedException(string, params object[]) {#constructor_3}

使用指定的错误消息初始化ObjectDisposedException类的新实例。

```csharp
public ObjectDisposedException(string format, params object[] parameters)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| format | String | 消息格式 |
| parameters | Object[] | 格式参数 |

### 也可以看看

* class [ObjectDisposedException](../../objectdisposedexception)
* 命名空间 [Aspose.Email.Exceptions](../../objectdisposedexception)
* 部件 [Aspose.Email](../../../)

---

## ObjectDisposedException(string, Exception) {#constructor_2}

初始化ObjectDisposedException类 的新实例，带有指定的错误消息和对导致此异常的内部异常。

```csharp
public ObjectDisposedException(string message, Exception innerException)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| message | String | 解释异常原因的错误消息。 |
| innerException | Exception | 导致当前异常的异常，如果没有内部异常，则为空引用（在 Visual Basic 中为空）被指定。 |

### 也可以看看

* class [ObjectDisposedException](../../objectdisposedexception)
* 命名空间 [Aspose.Email.Exceptions](../../objectdisposedexception)
* 部件 [Aspose.Email](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Email.dll -->
