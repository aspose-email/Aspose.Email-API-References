---
title: ListMailboxesAsync
second_title: Aspose.Email for .NET API 参考
description: 列出具有 smtp 地址的邮箱 注意返回的联系人的最大计数为 100这是使用 EWS 操作的限制
type: docs
weight: 440
url: /zh/net/aspose.email.clients.exchange.webservice/iasyncewsclient/listmailboxesasync/
---
## IAsyncEwsClient.ListMailboxesAsync method

列出具有 smtp 地址的邮箱。 注意：返回的联系人的最大计数为 100。这是使用 EWS 操作的限制。

```csharp
public Task<MapiContactCollection> ListMailboxesAsync(string filter = null, 
    CancellationToken cancellationToken = default)
```

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| filter | String | 过滤字符串 |
| cancellationToken | CancellationToken | 取消令牌 |

### 返回值

一个[`MapiContactCollection`](../../../aspose.email.mapi/mapicontactcollection)代表联系信息

### 也可以看看

* class [MapiContactCollection](../../../aspose.email.mapi/mapicontactcollection)
* interface [IAsyncEwsClient](../../iasyncewsclient)
* 命名空间 [Aspose.Email.Clients.Exchange.WebService](../../iasyncewsclient)
* 部件 [Aspose.Email](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Email.dll -->
