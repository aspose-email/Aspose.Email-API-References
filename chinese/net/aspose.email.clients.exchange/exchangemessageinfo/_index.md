...
| virtual [HasAttachments](../../aspose.email.clients.exchange/exchangemessageinfo/hasattachments) { get; } | 获取一个值，该值指示消息是否包含至少一个附件。 |
| [Headers](../../aspose.email.clients/messageinfobase/headers) { get; } | 获取电子邮件的标头。 |
| virtual [InternalDate](../../aspose.email.clients.exchange/exchangemessageinfo/internaldate) { get; } | 服务器上消息的内部日期和时间。 这不是 [RFC-2822] 标头中的日期和时间，而是反映收到消息的日期和时间。 - 对于通过 [SMTP] 传递的消息，这应该是 [SMTP] 定义的消息最终传递的日期和时间。 - 对于由 IMAP4rev1 COPY 命令传递的消息，这应该是源消息的内部日期和时间。 - 对于由 IMAP4rev1 APPEND 命令传递的消息，这应该是 APPEND 命令描述中指定的日期和时间。 - 所有其他情况均由实现定义。 |
| virtual [IsRead](../../aspose.email.clients.exchange/exchangemessageinfo/isread) { get; } | 获取消息是否已读的值 |
